...
- 👁️ **视觉分心判定**：学习时不自觉地拿起手机、趴下睡觉？系统会对摄像头与屏幕进行非线性稀疏采样，由强大的多模态 AI 进行**实时状态分析**，捕捉你的每一次分心。想开小差？你的伙伴会用语音**主动**提醒你，不想让它失望的话就接着学习吧。
...
> **Linux Docker 自动部署提示**：
> 你可以在项目根目录运行 `./deploy-linux.sh`。脚本会自动创建 `.env.docker`（若不存在）、自动生成 `AUTH_SECRET_KEY`（若仍是占位符）、自动从 Hugging Face 下载 Sherpa 模型，并启动前后端容器。
