...
def start_session_api(payload: SessionStartRequest, db: Session = Depends(get_db)):
    try:
...
def execute_penalty_api(payload: PenaltyExecuteRequest, db: Session = Depends(get_db)):
    try:
