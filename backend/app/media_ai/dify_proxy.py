...
class MockDifyClient:
    ...
    async def stream_chat(
        ...
        if has_system_result:
            ...
            keyword in user_text for keyword in ("看看", "看下", "帮我看", "分析一下")
        ):
    ...
    async def run_system_agent(
        ...
        str(inputs.get("supervision_state", "setup"))
        ...
        def minutes_from_text(default: int) -> int:
            ...
            match = re.search(
                r"(\d{1,3})\s*(分钟|分|min|mins|minute|minutes)",
                user_text,
                re.IGNORECASE,
            )
            if match:
        ...
        if any(
            ...
            for keyword in ("看看", "看下", "帮我看", "分析一下")
        ):
            sources: list[str] = []
            if any(
                keyword in user_text
                for keyword in ("桌面", "屏幕", "页面", "窗口", "代码", "文档", "应用")
            ):
            ...
            return {
                ...
                "system_events": [
                    f"[SYSTEM_EVENT: VISUAL_CONTEXT_REQUESTED, SOURCES: {','.join(sources)}]"
                ],
                "plan": None,
            }
        ...
        if any(
            keyword in user_text or keyword in lowered
            for keyword in ("结束", "停止", "不学了", "end", "stop", "finish")
        ):
            return {
                "action": "complete",
                "approved": True,
                ...
                "capture_sources": [],
                "system_events": [
                    "[SYSTEM_EVENT: SESSION_COMPLETED, SOURCE: system_agent]"
                ],
                "plan": None,
            }
        ...
        if any(
            keyword in user_text or keyword in lowered
            for keyword in ("继续", "恢复", "回来", "resume", "continue")
        ):
            return {
                "action": "resume",
                "approved": True,
        ...
        if any(
            ...
            approved = (
                urgent_reason or elapsed_seconds >= 300 or pause_requests_count == 0
            )
            return {
                "action": "pause",
                "approved": approved,
        ...
        if any(
                ...
                "tasks": [
                    {
                        "id": "t1",
                        "title": current_task
                        if current_task != "完成当前学习任务"
                        else user_text.strip() or current_task,
                        "completed": False,
            ...
            return {
                ...
                "capture_sources": [],
                "system_events": [
                    f"[SYSTEM_EVENT: PLAN_UPDATED, TITLE: {plan['tasks'][0]['title']}, TOTAL_MINUTES: {minutes}]"
                ],
                "plan": plan,
            }
        ...
        if any(
            keyword in user_text or keyword in lowered
            for keyword in ("开始", "开工", "进入监督", "start")
        ):
            minutes = minutes_from_text(
                max(int(inputs.get("suggested_focus_seconds", 1500) or 1500) // 60, 1)
            )
            return {
                "action": "start",
                "approved": True,
                ...
                "capture_sources": [],
                "system_events": [
                    f"[SYSTEM_EVENT: SESSION_START_REQUESTED, MINUTES: {minutes}]"
                ],
                "plan": None,
            }
...
class DifyClient:
    ...
    def __init__(self) -> None:
        ...
        self.file_upload_endpoint = os.getenv(
            "DIFY_FILE_UPLOAD_ENDPOINT", "/v1/files/upload"
        )
        self.vision_endpoint = os.getenv("DIFY_VISION_ENDPOINT", "/v1/workflows/run")
        self.system_agent_endpoint = os.getenv(
            "DIFY_SYSTEM_AGENT_ENDPOINT", "/v1/workflows/run"
        )
        self.chat_api_key = os.getenv("DIFY_CHAT_API_KEY", "")
        self.vision_api_key = os.getenv("DIFY_VISION_API_KEY", self.chat_api_key)
        self.system_agent_api_key = os.getenv(
            "DIFY_SYSTEM_AGENT_API_KEY", self.chat_api_key
        )
        ...
        self.retry_backoff_seconds = float(
            os.getenv("DIFY_HTTP_RETRY_BACKOFF_SECONDS", "0.8")
        )
    ...
    async def _sleep_before_retry(
        self, attempt: int, endpoint: str, exc: Exception
    ) -> None:
        delay = self.retry_backoff_seconds * attempt
        logger.warning(
            "Dify request failed, retrying endpoint=%s attempt=%s delay=%.2fs error=%s",
            endpoint,
            attempt,
            delay,
            exc,
    ...
    async def stream_chat(
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            if images:
                uploaded_files = await self._upload_chat_images(
                    client, session_id, images
                )
                if uploaded_files:
    ...
    async def _upload_chat_images(
        ...
        for index, image in enumerate(images):
            ...
            try:
                ...
                logger.warning(
                    "Failed to decode chat image payload for session %s", session_id
                )
                continue
    ...
    async def run_system_agent(
        ...
        logger.info(
            "calling Dify system workflow url=%s%s session_id=%s",
            self.base_url,
            self.system_agent_endpoint,
            session_id,
        )
        async with httpx.AsyncClient(timeout=self.system_agent_timeout) as client:
            ...
            for attempt in range(1, self.retry_attempts + 1):
                try:
                    ...
                    response.raise_for_status()
                    logger.info(
                        "Dify system workflow response received, session_id=%s status=%s",
                        session_id,
                    ...
                    await self._sleep_before_retry(
                        attempt, self.system_agent_endpoint, exc
                    )
    ...
    async def evaluate_start_readiness(
        ...
        _ = current_task
        has_camera = any(
            str(image.get("source", "")).lower() == "camera" for image in images
        )
        screen_metadata = next(
            (
                image.get("metadata")
                for image in images
                if str(image.get("source", "")).lower() == "screen"
                and isinstance(image.get("metadata"), dict)
            ),
            {},
        )
        screen_ok = (
            str((screen_metadata or {}).get("displaySurface", "")).lower() == "monitor"
        )
...
def get_dify_client() -> Any:
    ...
    Prefer app.media_ai.client_factory.get_agent_client for neutral naming.
    """
