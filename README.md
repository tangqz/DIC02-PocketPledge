...
#### TTS 与 大模型代理配置
默认使用本地代理调用各种大语言模型进行处理：
```env
# 开启本地代理
AGENT_BACKEND=local

# TTS 服务 (DashScope 用于 Qwen 流式语音合成)
...
pnpm run dev
```
启动完毕后浏览器打开相应本地地址 (`http://localhost:5173/` 等) 即可，前端默认会连接上 `ws://localhost:12393/ws`。
